...
pub fn init(http_client: Arc<HttpClientWithUrl>, cx: &mut App) {
    ...
    let auto_updater = cx.new(|cx| {
        ...
        if option_env!("OPPLA_UPDATE_EXPLANATION").is_none()
            && env::var("OPPLA_UPDATE_EXPLANATION").is_err()
            && poll_for_updates
        {
...
pub fn check(_: &Check, window: &mut Window, cx: &mut App) {
    if let Some(message) = option_env!("OPPLA_UPDATE_EXPLANATION") {
        drop(window.prompt(
            gpui::PromptLevel::Info,
            "Oppla was installed via a package manager.",
            Some(message),
            &["Ok"],
            cx,
        ));
    ...
    if let Ok(message) = env::var("OPPLA_UPDATE_EXPLANATION") {
        drop(window.prompt(
            gpui::PromptLevel::Info,
            "Oppla was installed via a package manager.",
            Some(&message),
...
async fn install_release_macos(
    ...
    let mount_path = temp_dir.path().join("Oppla");
