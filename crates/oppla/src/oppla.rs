...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &oppla_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    cx.on_action(|_: &oppla_actions::OpenTelemetryLog, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_telemetry_log_file(workspace, window, cx);
        });
    });
    cx.on_action(|&oppla_actions::OpenKeymap, cx| {
    ...
    cx.on_action(|_: &oppla_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
fn register_actions(
    ...
    workspace
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &oppla_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &oppla_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &oppla_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &oppla_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &oppla_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file::<ThemeSettings>(fs.clone(), cx, move |settings, _| {
        ...
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_on_click(|window, cx| {
                    window.dispatch_action(oppla_actions::OpenKeymap.boxed_clone(), cx);
            ...
            show_app_notification(id, cx, move |cx| {
                cx.new(|cx| {
                    MessageNotification::new(format!("Invalid user settings file\n{error}"), cx)
                        ...
                        .primary_on_click(|window, cx| {
                            window.dispatch_action(oppla_actions::OpenSettings.boxed_clone(), cx);
                            cx.emit(DismissEvent);
                        })
                })
            });
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &oppla_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open_local_file(
