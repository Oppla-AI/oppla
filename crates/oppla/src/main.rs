...
use crate::oppla::OpenRequestKind;
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Oppla failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Oppla failed to open a window: {e:?}. See https://zed.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Oppla failed to launch")
                        .body(Some(
                            format!(
...
pub fn main() {
    ...
    app.run(move |cx| {
        ...
        crate::oppla::component_preview::init(app_state.clone(), cx);
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| {
                        window.dispatch_action(
                            Box::new(oppla_actions::Extensions {
...
#[derive(Parser, Debug)]
#[command(name = "oppla", disable_version_flag = true)]
struct Args {
