//! Provides constructs for the Oppla app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("OPPLA_RELEASE_CHANNEL").or_else(|_| env::var("ZED_RELEASE_CHANNEL")).unwrap_or_else(|_| {
            include_str!("../../oppla/RELEASE_CHANNEL")
                .trim()
                .to_string()
        })
    } else {
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Oppla-Editor-Dev",
        ReleaseChannel::Nightly => "Oppla-Editor-Nightly",
        ReleaseChannel::Preview => "Oppla-Editor-Preview",
        ReleaseChannel::Stable => "Oppla-Editor-Stable",
    }
}

/// The Git commit SHA that Oppla was built at.
...
pub enum ReleaseChannel {
    ...
    ///
    /// Used for local debug builds of Oppla.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "Oppla Dev",
            ReleaseChannel::Nightly => "Oppla Nightly",
            ReleaseChannel::Preview => "Oppla Preview",
            ReleaseChannel::Stable => "Oppla",
        }
    }
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Oppla on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "dev.oppla.Oppla-Dev",
            ReleaseChannel::Nightly => "dev.oppla.Oppla-Nightly",
            ReleaseChannel::Preview => "dev.oppla.Oppla-Preview",
            ReleaseChannel::Stable => "dev.oppla.Oppla",
        }
    }
